...
func (r *tokenRepository) Create(ctx context.Context, token *domain.Token) (string, error) {
	...
	query := `
	INSERT INTO refresh_tokens (id, user_id, refresh_token, user_agent, ip_address, expires_at, created_at)
	VALUES (:id, :user_id, :refresh_token, :user_agent, :ip_address, :expires_at, :created_at)
	`
